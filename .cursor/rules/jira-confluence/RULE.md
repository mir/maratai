...
# Fetch a ticket
uv run --directory .cursor/rules/jira-confluence scripts/jira.py get PROJ-123

# Search with JQL
uv run --directory .cursor/rules/jira-confluence scripts/jira.py search "project=DEMO AND status='In Progress'"
...
uv run --directory .cursor/rules/jira-confluence scripts/jira.py export "project=DEMO" --format markdown --output-dir ./exports/

# Export to stdout (yaml default)
uv run --directory .cursor/rules/jira-confluence scripts/jira.py export "project=DEMO"

# Export to JSON
uv run --directory .cursor/rules/jira-confluence scripts/jira.py export "project=DEMO" --format json
```
...
uv run --directory .cursor/rules/jira-confluence scripts/jira.py comment PROJ-123 "This is my comment"

# Get available transitions
uv run --directory .cursor/rules/jira-confluence scripts/jira.py transitions PROJ-123

# Transition ticket status
uv run --directory .cursor/rules/jira-confluence scripts/jira.py transition PROJ-123 "In Progress"

# Or by transition ID
uv run --directory .cursor/rules/jira-confluence scripts/jira.py transition PROJ-123 --id 21
```

## Create ticket
```bash
uv run --directory .cursor/rules/jira-confluence scripts/jira.py create PROJ --type Story --summary "My new story"
# With more options
uv run --directory .cursor/rules/jira-confluence scripts/jira.py create PROJ --type Bug --summary "Bug title" --description "Details here" --priority High --labels "bug,urgent"
# With custom fields (for project-specific required fields)
uv run --directory .cursor/rules/jira-confluence scripts/jira.py create PROJ --type Story --summary "My story" --field "Story Points=5" --field "Team=Platform"
```

## Edit ticket
```bash
uv run --directory .cursor/rules/jira-confluence scripts/jira.py edit PROJ-123 --summary "Updated title"
# Update multiple fields
uv run --directory .cursor/rules/jira-confluence scripts/jira.py edit PROJ-123 --summary "New title" --description "New description"
# Update custom fields
uv run --directory .cursor/rules/jira-confluence scripts/jira.py edit PROJ-123 --field "Story Points=8" --field "customfield_10001=value"
```

## Assign ticket
```bash
uv run --directory .cursor/rules/jira-confluence scripts/jira.py assign PROJ-123 <account_id>
# Assign to yourself
uv run --directory .cursor/rules/jira-confluence scripts/jira.py assign PROJ-123 --me
# Unassign
uv run --directory .cursor/rules/jira-confluence scripts/jira.py assign PROJ-123 --unassign
```
...
```bash
uv run --directory .cursor/rules/jira-confluence scripts/jira.py me
```

## Lookup user by name or email
```bash
uv run --directory .cursor/rules/jira-confluence scripts/jira.py lookup "John Doe"
```
...
```bash
# Fetch a page
uv run --directory .cursor/rules/jira-confluence scripts/confluence.py get 123456789
...
```bash
# Export to markdown files
uv run --directory .cursor/rules/jira-confluence scripts/confluence.py export "space=TEAM" --format markdown --output-dir ./exports/

# Export to stdout (yaml default)
uv run --directory .cursor/rules/jira-confluence scripts/confluence.py export "type=page AND title~'architecture'"
...
```bash
uv run --directory .cursor/rules/jira-confluence scripts/jira.py create PROJ --type Story --summary "Title" --field "customfield_10001=5"
```
